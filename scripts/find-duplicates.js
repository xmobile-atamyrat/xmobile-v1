...
async function checkDuplicates() {
    ...
    categories.forEach((cat) => {
        ...
        catSlugs
            .get(slug)
            .push({ id: cat.id, originalDbName: cat.name, bareName });
    });
    ...
    catSlugs.forEach((items, slug) => {
        if (items.length > 1) {
            ...
            items.forEach((item) => console.log(`   - ID: ${item.id} | Name: ${item.bareName}`));
            console.log('---');
        }
    });
    if (!hasCatDuplicates)
        console.log('✅ No category duplicate slugs found!\n');
    // 2. Check Products
    ...
    if (!hasProdDuplicates)
        console.log('✅ No product duplicate slugs found!\n');
    if (hasCatDuplicates || hasProdDuplicates) {
        ...
        console.log('Once you confirm they are unique, we will proceed with the schema migration.');
    }
    else {
