# Documentation

Concise reference for contributors. Start here, then jump to the topic you need.

| Doc | Contents |
|-----|----------|
| [**Architecture**](architecture.md) | High-level: web app, API, WebSocket server, batch-runner, mobile. How they connect. |
| [**Codebase structure**](codebase-structure.md) | Directory layout: `src/pages`, `src/lib`, `ws-server`, `prisma`, `scripts`, `mobile`. |
| [**API**](api.md) | REST conventions, auth, main endpoint groups. |
| [**Database**](database.md) | Prisma schema, main models, migrations. |
| [**Features**](features.md) | Auth, cart/checkout, orders, admin, procurement, chat, FCM, i18n. |
| [**Development**](development.md) | Env, running WS/batch-runner, lint, workflow. |
| [**Environment**](environment.md) | All env vars with short descriptions. |
| [**FCM setup**](fcm-setup.md) | Firebase and VAPID setup for push notifications. |
| [**Deployment**](deployment.md) | Build and Deploy, Deploy Batch Runner, Daily Backup to Drive (GitHub Actions). |
| [**SEO**](seo-knowledge.md) | Meta tags, structured data, sitemap, and SEO implementation. |
| [**Mobile App**](mobile/README.md) | React Native WebView wrapper for Android and iOS. |
