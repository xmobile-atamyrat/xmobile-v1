...
export default function Footer () {
  ...
  useEffect(() => {
    setMaxHeight(isMdUp ? (window.innerHeight- appBarHeight) : (window.innerHeight - mobileAppBarHeight))
  }, [isMdUp])
  ...
  return (
      ...
      >
        ...
        <Box className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3">
          ...
          <Box className="flex flex-col pb-6">
            {/* logo */}
              <Typography className="pl-6" variant="h4" style={{ fontWeight: 'bold' }} paddingBottom={5}>
                <Link href="/">
                  X-Mobile
                </Link>
              </Typography>
            ...
            >
              ...
              <Typography fontWeight={400} fontSize={isMdUp ? 16 : 14}>
                <Link href={'https://maps.app.goo.gl/sYc6VJSSFJW1aUd76'} target='_blank'>{t('address')}</Link>
              </Typography>
            </Box>
          </Box>
          ...
          <Box className="flex flex-col pb-6">
            ...
            <Box className="">
              {[0, 1, 2].map((number) => (
                <Typography margin={2} key={phoneNumbers[number]} fontSize={isMdUp ? 16 : 14}>
                  <b>{t('mobilePhone')} {number + 1}: </b>
                  <a href={`tel:${phoneNumbers[number]}`} className='pl-3'/* onClick={() => handleClick(number)} */ >{phoneNumbers[number]}</a>
                </Typography>
              ))}
          ...
          <Box className="flex flex-col pb-6">
            ...
            <Box className="flex flex-col">
              <Link target="_blank" href={'https://www.instagram.com/xmobiletm/'}>
                <IconButton>
      ...
      <Box>
        <Typography className="bold flex justify-center" fontWeight={'bold'} fontSize={14}>
          &copy; 2025 X-Mobile
        </Typography>
      </Box>
